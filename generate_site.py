...
@media (min-width:900px){
  ...
  /* ===== 営業時間 & 買取受付時間（SPは2行の列揃え／PCは1行「/」区切り） ===== */
  #home .info-time{ display:block; }
  #home .info-time .row{
    display:grid;
    grid-template-columns:auto 1fr; /* 左:ラベル / 右:時間（数字列を縦に揃える） */
    column-gap:12px;
    align-items:baseline;
  }
  #home .info-time .label{ white-space:nowrap; }
  #home .info-time .time{ text-align:left; }
  @media (min-width:900px){
    #home .info-time{ display:flex; align-items:center; gap:.4em; flex-wrap:wrap; }
    #home .info-time .row{ display:inline-flex; gap:.35em; }
    #home .info-time .row + .row::before{ content:" / "; margin:0 .2em; }
  }
...
@media (max-width:900px){
  ...
  nav{
    order:1; width:100%; display:flex; justify-content:center; gap:6px;
  ...
  #events .event-row .right .event-schedule,
  #events .event-row .right > img{
    margin-top:0; width:92%; max-width:520px; margin-left:auto; margin-right:auto;
    clip-path:none; position:static; z-index:auto;
  }
  #events .event-row .right .event-schedule img{ width:100%; height:auto; clip-path:none; }

  /* HOME：POP画像の下に出る微小な空白を消す（SP） */
