# -*- coding: utf-8 -*-
# site/ に index.html / mycalinks.html を生成（白背景・黒文字）
# - ヘッダー店名テキスト削除（ロゴのみ）
# - ナビ左寄せ：HOME | 大会・イベント | 店頭買取 | 郵送買取 | デュエルスペース | アクセス | 通販 | X | LINE
# - HOMEの pop_mycalinks クリックで「郵送買取 (#buy)」へ
# - 大会イベントはスケジュール主体（黒枠・白背景）
# - 店頭買取2枚も黒枠・白背景
# - カルーセル（PC: 矢印/広いクリック, SP: スワイプ・25pxしきい値）
# - スマホ：HOME右カラム画像の下余白除去、営業時間/買取受付の数字縦揃え
...
MAP_EMBED_SRC = (
    ...
    "1d12959.948330384788!2d139.7712742631493!3d35.70193548613032!2m3!"
    "1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!"
    "1s0x60188c1dd8a0a351%3A0x521f149db8f31043!"
    "2z44CSMTAxLTAwMjEg5p2x5Lqs6YO95Y2D5Luj55Sw5Yy65aSW56We55Sw77yT5LiB55uu77yR77yV4oiS77yV!"
...
LINE_ICON_CANDIDATES = [
    "LINE.png","line.png","LINE.webp","line.webp",
    "LINE.jpg","line.jpg","LINE.jpeg","line.jpeg","LINE.svg","line.svg"
]

HOME_MAX_SLIDES = 7
TITLES_DEF = [
    {"name":"デュエル・マスターズ","key":"duel","url":"https://dm.takaratomy.co.jp/",
     "imgs":["DUELMASTERS.webp","DUELMASTERS.png","duelmasters.webp","duelmasters.png","duelmasters.jpg"]},
    {"name":"ポケモンカードゲーム","key":"poke","url":"https://www.pokemon-card.com/",
     "imgs":["pokemon.png","pokemon.webp","pokemon.jpg","POKEMON.png"]},
    {"name":"Disney Lorcana","key":"lorc","url":"https://www.takaratomy.co.jp/products/disneylorcana/",
     "imgs":["LORCANA.png","LORCANA.webp","lorcana.png","lorcana.jpg"]},
    {"name":"ヴァイスシュヴァルツ","key":"weiss","url":"https://ws-tcg.com/",
     "imgs":["WEISSCHWARZ.png","WEISSSCHWARZ.png","weissschwarz.png","weiss.png"]},
]
...
@media (min-width:900px){ header .x-icon-img, header .line-icon-img{ width:34px; height:34px; } }
...
.ctrl{ position:absolute; top:50%; transform:translateY(-50%); background:rgba(17,17,17,.6); color:#fff; border:none; width:44px; height:44px; border-radius:50%; cursor:pointer; z-index:5; }
...
@media (max-width:900px){ .ctrl{ display:none !important; } }
...
.home-card img{ width:100%; height:auto; object-fit:cover; object-position:center; }
...
.titles h2{ margin:0 0 8px; font-size:1.25rem; }
.title-grid{ display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:14px; }
...
#events .event-row .right .event-schedule{
  width:80%; margin-top: calc(-1 * var(--schedule-lift));
  border:1px solid #000; border-radius:12px; background:#fff; overflow:hidden; box-shadow:0 2px 6px rgba(0,0,0,.12);
}
#events .event-row .right .event-schedule img{
  width:100%; height:auto; display:block; border:none; border-radius:0; background:#fff;
...
/* 営業時間/買取受付：2段＆数字揃え */
.info-time{ display:grid; gap:4px; }
.info-time .row{ display:grid; grid-template-columns:4.2em 1fr; align-items:baseline; }
.info-time .label{ text-align:right; white-space:nowrap; }
.info-time .time{ font-variant-numeric: tabular-nums; letter-spacing:.02em; }
...
.duel-stack{ display:flex; flex-direction:column; gap:10px; align-items:flex-end; }
.duel-panel{ width:100%; max-width:620px; border:1px solid rgba(0,0,0,.12); border-radius:12px; padding:10px 12px; background:rgba(0,0,0,.04); }
.duel-panel h3{ margin:0 0 4px; font-size:1.02rem; }
...
#buy .buy-cta{ margin-top:0; border:none !important; background:transparent !important; }
#buy .buy-cta-link{ position:relative; display:block; width:100%; aspect-ratio: var(--cta-ar); overflow:hidden; }
#buy .buy-cta-img{ position:absolute; inset:0; width:100%; height:100%; display:block; object-fit:cover; clip-path: inset(10% 0 10% 0); transform: translate(1.5%,-8%) scale(1.2); transform-origin:center; }
...
@media (min-width:900px){
  #events,#storebuy,#buy{ width:80%; margin-left:16%; margin-right:0; }
...
@media (max-width:900px){
  ...
  #events .event-row .right .event-schedule{ margin-top:0; width:92%; max-width:520px; margin-left:auto; margin-right:auto; clip-path:none; position:static; z-index:auto; }
  ...
  /* BUY：縦積み＆間延び対策 */
  #buy .buy-layout{ grid-template-columns:1fr; gap:8px; }
  ...
  /* yusoudragon：低め比率で上下トリム */
  #buy .buy-cta{ margin-top:6px !important; }
  #buy .buy-cta-link{ aspect-ratio:16/7 !important; overflow:hidden !important; }
  #buy .buy-cta-img{
    position:absolute !important; inset:0 !important; width:100% !important; height:100% !important;
    object-fit:cover !important; object-position:48% center !important; clip-path: inset(12% 0 12% 0) !important; transform: translateX(1.5%) scale(1.2) !important;
  }
  ...
  /* 営業時間/買取受付の数字カラム幅微調整 */
  .info-time .row{ grid-template-columns:4.1em 1fr; }
}

/* SPの buy-table 列幅 28:72 */
@media (max-width:900px){
...
<main>
  ...
  <section id="events" class="section">
    <div class="container">
      <h2>大会・イベント</h2>
      <p class="event-intro">週のどの日でも、CLIMAXのデュエルスペースは大会が繰り広げられる舞台。
ここはただの対戦会場ではなく、“本気のデュエリスト”が集まる特別な場所です。
ひとりでも、仲間とでも、あなたの挑戦を待っています！</p>
      <div class="event-row">
        <div class="left"><img src="${EVENTS_PHOTO}" alt="デュエルスペースの様子"></div>
        <div class="right">
          <div class="event-schedule"><img src="${SCHEDULE_IMG}" alt="イベントスケジュール" loading="lazy"></div>
        </div>
      </div>
    </div>
  </section>
  ...
  <section id="storebuy" class="section">
    <div class="container">
      ...
      <p class="storebuy-note">
        店頭買取には <a class="myca-link" href="https://myca.cards/" target="_blank" rel="noopener"><strong>Mycalinks アプリのインストール</strong></a> が必要です。以下のPOPを確認してください。
      </p>
  ...
  <section id="buy" class="section">
    <div class="container">
      <h2>郵送買取</h2>
      <p class="buy-note">
        郵送買取には <a class="myca-link" href="https://myca.cards/" target="_blank" rel="noopener"><strong>Mycalinks アプリのインストール</strong></a> が必要です。手順や最新の買取価格は下記をご確認ください。
      </p>
      ...
      <div class="buy-notes panel">
        ...
        <ul style="margin:0; padding-left:1.2em; line-height:1.6; color:#111;">
          ...
          <li>レアとノーマルが混在する場合は、あらかじめレアとノーマルを分けてから梱包・発送してください。<br>（※ノーマルに混在しているレアカードなどは一括でノーマルカード査定となる可能性があります。）</li>
      ...
      <div class="duel-grid">
        ...
        <div class="duel-stack">
          <div class="duel-panel duel-panel--compact">
            <div class="duel-headline" style="font-weight:900;font-size:1.1em;letter-spacing:.02em;white-space:nowrap;">FREE毎日開放</div>
            <p style="margin-top:10px;white-space:pre-line">デュエルスペースはいつでも無料で使えて、毎週各種大会も開催しています！
腕試しや仲間増やしの場にもピッタリ！</p>
          </div>
...
// ==== HOMEカルーセル（PC:クリック/キー、SP:スワイプ）+右POP高さ同期 ====
(function(){
  ...
  slides.forEach(s=>{
    ...
    img.setAttribute('draggable','false'); img.style.userSelect='none';
  });

  function slideHeight(idx){
    const s = slides[idx], img = s.querySelector('img'); if(!img) return 0;
    const w = carousel.clientWidth; if(img.naturalWidth===0) return 0;
  ...
  function syncSide(h){
    ...
    if(window.matchMedia("(max-width:900px)").matches){ cards.forEach(c=>c.style.height="auto"); return; }
  ...
  const THRESHOLD_PX = 25, LOCK_PX = 8;
  ...
  const onMove = (x,y,ev)=>{
    ...
    if(axis===null && (Math.abs(dx)>LOCK_PX || Math.abs(dy)>LOCK_PX)){ axis = Math.abs(dx) > Math.abs(dy) ? 'x' : 'y'; }
    if(axis==='x'){
  ...
  const onUp = ()=>{
    if(!isDown) return;
    isDown = false;
    if(axis==='x' && Math.abs(deltaX) > THRESHOLD_PX){ (deltaX < 0) ? go(i+1) : go(i-1); }
    else{ snap(); }
    autoStart();
  };
  ...
  window.addEventListener('touchmove',  (e)=>{ const t=e.touches[0]; if(t) onMove(t.clientX, t.clientY, e); }, {passive:false});
  ...
  slides.forEach((s,idx)=>{ const img=s.querySelector('img'); if(!img) return; if(img.complete){ if(idx===0) adjustAll(); } else img.addEventListener('load', ()=>{ if(idx===0) adjustAll(); }, {once:true}); });
...
(OUT / "index.html").write_text(
    INDEX_HTML.safe_substitute(
        ...
        EVENTS_PHOTO=EVENTS_PHOTO, MAIL_POP_HTML=MAIL_POP_HTML,
        BUY_CTA_HTML=BUY_CTA_HTML, LINE_ICON_HTML=LINE_ICON_HTML,
    ),
    encoding="utf-8"
)

(OUT / "mycalinks.html").write_text(
    MYCA_HTML.safe_substitute(
