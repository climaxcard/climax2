...
@media (max-width:900px){
  ...
  .carousel .track{ touch-action: pan-y; cursor: grab; }
}
...
#events .event-row .right .event-schedule img{
  width:100% !important; height:auto !important; display:block; border:none; border-radius:0; background:#fff;
...
@media (min-width:900px){ #buy .buy-notes.panel{ width:85.3%; margin:-8px 10% 0 0; } }
@media (max-width:899px){ #buy .buy-notes.panel{ width:100%; margin:12px 0 0; } }
@media (min-width: 900px){ #storebuy, #buy { width: 80%; margin-left: 16%; margin-right: 0; } }
@media (min-width: 900px){
...
/* === BUY（郵送買取）SP：縦並び & yusoudragon 右寄せ＋上下トリム（復元版） === */
@media (max-width: 900px){
  ...
  /* ← ここが重要：SP 時 yusoudragon を低め比率に・右寄せ・上下トリム */
  ...
  #buy .buy-cta-img{
    ...
    object-fit: cover !important; object-position: 48% center !important;
    transform: translateX(1.5%) scale(1.2) !important; transform-origin: center center !important;
    clip-path: inset(12% 0 12% 0) !important; image-rendering: auto !important;
  }

  #buy .buy-notes.panel{ width: 90% !important; margin: 12px 12% 0 0 !important; }
}

/* === SP（～900px）: HOME情報テーブル 行合わせ（統合・最終適用） === */
@media (max-width: 900px){
  ...
  #home .info-table th{ width: 6.6em !important; padding: 5px 8px !important; white-space: nowrap; }
  #home .info-table td{ padding: 5px 8px !important; }
  /* 2行クランプ（営業時間・買取受付時間の行合わせ） */
  #home .info-table .clamp-2{
    display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2;
    overflow: hidden; line-height: 1.28; word-break: break-word;
  }
  /* 3行クランプ（アクセス文など） */
...
@media (max-width:900px){
  ...
  #events .event-row .right .event-schedule{ margin-top:0; width:92%; max-width:520px; margin-left:auto; margin-right:auto; }
...
/* ==== HOMEカルーセル（PC:矢印/どこでもクリック・SP:スワイプ） ==== */
(function(){
  ...
  slides.forEach(s=>{ const img=s.querySelector('img'); if(img){ img.setAttribute('draggable','false'); img.style.userSelect='none'; } });
  ...
  let i=0, timer=null; const DURATION = 500;
  ...
  const onDown = (x,y)=>{ isDown = True=false; }; // (dummy to preserve structure)
})();
</script>
</body>
</html>
